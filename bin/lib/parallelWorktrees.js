...
function createWorktrees(targetDir, workers, log = console.log) {
  ...
  for (const worker of workers) {
    if (worker.name === MAIN_WORKER_NAME) {
      ...
      if (fs.existsSync(legacyMainWorktreePath)) {
        log(`  ${C.yellow}!${C.reset} legacy worktree '${MAIN_WORKER_NAME}' 감지 — ${C.cyan}npx sleepcode run --clean${C.reset} 권장`);
      }
      log(`  ${C.green}✓${C.reset} ${worker.name} ${C.dim}(${currentBranch})${C.reset} — ${worker.remaining}개 태스크 ${C.dim}[현재 브랜치]${C.reset}`);
      created.push({
        name: worker.name,
        path: targetDir,
    ...
    if (fs.existsSync(wtPath)) {
      log(`  ${C.dim}-${C.reset} ${worker.name} ${C.dim}(이미 존재)${C.reset}`);
    ...
    try {
      ...
      try {
        ...
        log(`  ${C.red}✗${C.reset} ${worker.name}: ${e.message}`);
        continue;
      }
    }
    ...
    log(`  ${C.green}✓${C.reset} ${worker.name} ${C.dim}(${branch})${C.reset} — ${worker.remaining}개 태스크`);
    created.push({ name: worker.name, path: wtPath, branch, tasksPath: wtTasksPath });
  }
...
function cleanupWorktrees(targetDir, workers, log = console.log) {
  ...
  if (workers) {
    for (const worker of workers) {
      if (worker.name === MAIN_WORKER_NAME) {
        if (fs.existsSync(mainTasksPath)) {
          try {
            ...
            log(`  ${C.green}✓${C.reset} ${worker.name} task 파일 제거`);
          } catch (e) {
            log(`  ${C.red}✗${C.reset} ${worker.name}: ${e.message}`);
          }
        }
      ...
      try {
        ...
        log(`  ${C.green}✓${C.reset} ${worker.name} worktree 제거`);
      } catch (e) {
        log(`  ${C.red}✗${C.reset} ${worker.name}: ${e.message}`);
      }
    }
    ...
    if (!fs.existsSync(wtBase)) {
      if (!fs.existsSync(mainTasksPath)) {
        log(`${C.dim}정리할 worktree가 없습니다.${C.reset}`);
        return;
      }
    }
    if (fs.existsSync(wtBase)) {
      ...
      for (const dir of dirs) {
        ...
        try {
          execSync(`git worktree remove "${wtPath}" --force`, { cwd: targetDir, stdio: 'pipe' });
          log(`  ${C.green}✓${C.reset} ${dir} worktree 제거`);
        } catch (e) {
          log(`  ${C.red}✗${C.reset} ${dir}: ${e.message}`);
        }
      }
    }
    if (fs.existsSync(mainTasksPath)) {
      try {
        fs.unlinkSync(mainTasksPath);
        log(`  ${C.green}✓${C.reset} main task 파일 제거`);
      } catch (e) {
        log(`  ${C.red}✗${C.reset} main task 파일 제거 실패: ${e.message}`);
      }
    }
  }
